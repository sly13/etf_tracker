...
/**
 * Скрипт синхронизации 5-минутных свечей ETHUSDT из Binance Spot API в PostgreSQL
...
const config = {
	...
	startFrom: process.env.START_FROM || '2017-08-17T00:00:00Z', // ETH начал торговаться на Binance в августе 2017
...
async function fetchKlines(startTime, endTime, retryCount = 0) {
	try {
		...
		console.log(`📡 Запрос данных ETH: ${startTime.toISOString()} - ${endTime.toISOString()}`);
		...
		if (retryCount < config.maxRetries) {
			...
			console.log(`⚠️ Ошибка запроса ETH (попытка ${retryCount + 1}/${config.maxRetries}): ${error.message}`);
		...
		throw new Error(`Не удалось получить данные ETH после ${config.maxRetries} попыток: ${error.message}`);
	}
}
...
/**
 * Получение последней даты из БД для ETH
 */
async function getLastOpenTime() {
	try {
		...
		console.error('❌ Ошибка получения последней даты ETH:', error.message);
		throw error;
	}
}

/**
 * Массовая вставка/обновление данных ETH
 */
	...
	try {
		...
		for (const candle of candles) {
			try {
				...
				console.error(`❌ Ошибка upsert свечи ETH:`, {
					symbol: candle.symbol,
					interval: candle.interval,
		...
		console.error('❌ Ошибка upsert данных ETH:', error.message);
		throw error;
	}
}

/**
 * Основная функция синхронизации ETH
 */
async function syncKlines() {
	...
	try {
		console.log('🚀 Запуск синхронизации ETH свечей...');
		...
		if (startTime >= now) {
			console.log('✅ Данные ETH актуальны, синхронизация не требуется');
			return;
		}
		...
		while (startTime < now) {
			...
			try {
				...
				if (klines.length === 0) {
					console.log(`📭 Батч ${batchCount}: нет данных ETH`);
					break;
				}
				...
				console.log(`✅ Батч ${batchCount}: ${upserted} ETH свечей, последняя: ${lastCandle.closeTime.toISOString()}`);
				...
				console.error(`❌ Ошибка в батче ETH ${batchCount}:`, error.message);
				throw error;
			}
		}

		console.log('🎉 Синхронизация ETH завершена!');
		console.log(`📊 Итого обработано батчей: ${batchCount}`);
		console.log(`📈 Итого upsert ETH свечей: ${totalUpserted}`);

	} catch (error) {
		console.error('💥 Критическая ошибка синхронизации ETH:', error.message);
		throw error;
	}
}
...
async function main() {
	try {
		await syncKlines();
		console.log('✅ Скрипт ETH выполнен успешно');
		process.exit(0);
	} catch (error) {
